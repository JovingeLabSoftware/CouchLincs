 ...
 * @example lincs.getPlate("1").then(function(x) { console.log(x) });
 */
LINCS.prototype.getByDistilID = function(ids, fields, cb) {
	var deferred = Q.defer();
  ids = [].concat(ids);
  var q = this._prepare(fields, {distil_id: ids});
  this._execute(q, deferred);
  deferred.promise.nodeify(cb);
  return deferred.promise;
};
...
LINCS.prototype._prepare = function(select, primary, secondary, skip, limit) {
  ...
  q = `SELECT META().id, ${select.join(", ")} FROM LINCS `+
          "WHERE " + this._whereStmt(primary_field, primary_value);
